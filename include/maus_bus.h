...
typedef struct __attribute__ ((packed)) {
    ...
    // 0x10 - 0x27
    char vendor_name[MAUS_BUS_VENDOR_MAX_LENGTH + 1];                   // User-defined Vendor Name for the product. Null-terminated.
    // 0x28 - 0x3f
    char product_name[MAUS_BUS_PRODUCT_MAX_LENGTH + 1];                  // User-defined display name for the product. Null-terminated.
} maus_bus_device_t;
 ...
 */
typedef maus_bus_err_t (*maus_bus_master_write_fn)(uint8_t address, uint8_t subaddress, uint8_t *data, size_t len);

/**
 * @brief Read callback to interface with your specific hardware setup.
 */
typedef maus_bus_err_t (*maus_bus_master_read_fn)(uint8_t address, uint8_t subaddress, uint8_t *data, size_t len);
 ...
 */
typedef void (*maus_bus_scan_callback_t)(maus_bus_device_t* device, maus_bus_address_t address, void* ptr);
...
typedef void (*maus_bus_enumeration_callback_t)(maus_bus_driver_t *driver, maus_bus_device_t *device, maus_bus_address_t address, void *ptr);
